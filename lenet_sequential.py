...
class LeNet5(nn.Module):
    def __init__(self, in_channels=3, model_name='LeNet5_v1', nclasses=10):
        ...
        if model_name == 'LeNet5_v2':
            ...
            self.features = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=6, kernel_size=5, padding=0),
                # input:32*32*3 output:28*28*6
                nn.MaxPool2d(kernel_size=2, stride=2),
                nn.ReLU(),
                nn.Conv2d(in_channels=6, out_channels=16, kernel_size=5),  # input:14*14*6 output:10*10*16
                nn.MaxPool2d(kernel_size=2, stride=2),
                nn.ReLU(),
            )
            self.classifier = nn.Sequential(
                nn.Linear(in_features=16 * 5 * 5, out_features=120),  # input:5*5*16 output:120
                nn.ReLU(),
                nn.Linear(in_features=120, out_features=84),  # input:120 output:84
                nn.ReLU(),
                nn.Linear(in_features=84, out_features=10),  # input:84 output:10
            )
